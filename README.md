...
- `lib/manager-intake.js` — `resolveManagerIntake()` (resolve buddy/mentor emails to real employees for an in-memory merge), `validateMentorSelection()` (mentor must be the employee's own manager or same `team_id`)
