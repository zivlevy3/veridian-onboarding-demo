...
// - runs on the raw Process Expert plan BEFORE validatePlanOrThrow, so a violation the
// model generated gets corrected here instead of discarding the whole plan and
// regenerating from scratch. See MEMORY.md for why: once the JSON-shape failure class
// was eliminated (2026-08-19, forced structured tool use), cap violations turned out to
// be the dominant real-world failure mode - "throw the whole plan away and re-roll the
// dice" was expensive and, empirically, not reliable even across 4 attempts. This is
...
//   dependency), the item is left where it is; if that leaves a genuine, irreducible
//   violation (mandatory items alone already exceed a cap - rare, not what's been
//   observed in practice), it surfaces to validatePlanOrThrow as a real error - the one
//   case this still can't paper over.
...
// Runs on the raw Process Expert plan, before validatePlanOrThrow. Mutates `plan` in
// place (weeks[].items arrays) and returns { plan, log, fixed } - `log` is the full list
// of moves made (empty if nothing needed fixing), `fixed` is whether every violation
// found was successfully resolved. `fixed: false` means a real, irreducible violation
// remains - validatePlanOrThrow still throws on it, same as before this existed.
function rebalancePlan(plan) {
  ...
  const stillCapViolating = plan.weeks.some(
    (week) => countSharedCapMeetings(week).length > SHARED_MEETING_CAP || countWeeklyLoadUnits(week) > LOAD_UNIT_CAP
  );
  const stillMisplacedDirectReport = plan.weeks.some(
    (week) => week.weekNumber > DIRECT_REPORT_MAX_WEEK && week.items.some((item) => item.facilitatorType === 'direct_report')
  );

  return { plan, log, fixed: !stillCapViolating && !stillMisplacedDirectReport };
}
