...
function buildEmployeeContext(db, employeeId) {
  ...
  if (team) {
    ...
    const isExecutiveTitle = (t) => /^(chief|vp\s|vice president|ceo\b|cfo\b|cto\b|coo\b|cro\b|ciso\b)/i.test(t.job_title);
    const hasExecutiveMember = teammates.some((t) => t.department === 'Executive' || isExecutiveTitle(t));
  ...
  // hrbp_email (HR Business Partners are assigned a real portfolio of managers), so this
  // is empty for almost everyone and non-empty (with real people) for actual HRBPs. Not
  // invented for new hires with no assigned portfolio yet - it's a real query, same as
  // directReports, not a role-specific guess.
