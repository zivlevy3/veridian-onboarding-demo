...
// 2. A teammate (same team + department) who is at the new hire's office - a genuine
//    peer, preferred over the manager below when one actually exists.
// 3. The direct manager, if they're at the new hire's office - a real, meaningful
//    relationship, and a better fallback than a random stranger from an unrelated team.
//    Only reached when no real teammate exists (e.g. a small team) - never preferred
//    over one. Tagged its own `reason: 'manager'`, not `'teammate'`, so Process Expert
//    folds the tour into the existing direct-manager item instead of giving the same
//    person two contradictory labels (direct manager everywhere else, generic "team
//    member" for the tour) in the same plan.
// 4. Any other employee at the new hire's office, as a last resort.
...
// whatever's in the DB - callers should pass the already-merged value. Takes
// `managerEmail` too, for the same reason `context.employee` doesn't carry it.
function resolveOfficeTourGuide(db, employee, humanBuddy, managerEmail) {
  ...
  const teammate = db
    .prepare(
      `SELECT * FROM employees WHERE team = ? AND department = ? AND location = ? AND email != ? AND email != ? LIMIT 1`
    )
    .get(employee.team, employee.department, employee.location, employee.email, managerEmail || '');
  if (teammate) return { ...personSummary(teammate), reason: 'teammate' };

  if (managerEmail) {
    const managerRow = db.prepare('SELECT * FROM employees WHERE email = ?').get(managerEmail);
    if (managerRow && managerRow.location === employee.location) {
      return { ...personSummary(managerRow), reason: 'manager' };
    }
  }

  const officeMate = db
    .prepare(`SELECT * FROM employees WHERE location = ? AND email != ? AND email != ? LIMIT 1`)
    .get(employee.location, employee.email, managerEmail || '');
...
function buildEmployeeContext(db, employeeId) {
  ...
  if (!humanBuddy) {
    gaps.push(
      'human_buddy_email is not set - Buddy has not been assigned yet for this hire ' +
