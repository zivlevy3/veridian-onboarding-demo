...
// -> validate (halts here on failure) -> Content Writer -> persist. `db` must be opened
// writable (openDb({ writable: true })) - this saves the plan via lib/persistence.js
// rather than returning a transient, unsaved object.
async function runOrchestrator(db, employeeId, intakeInput = {}) {
  ...
  const contentExpertResult = await runContentExpert(finalMergedContext);
  ...
  const plan = await runProcessExpert(contextForProcessExpert);
  validatePlanOrThrow(plan);

  console.log('Calling Content Writer agent...');
  const content = await runContentWriter(plan, contextForProcessExpert);
