...
// Builds a plain-text instruction from Gatekeeper's own blocking issues, for
// lib/content-writer-agent.js's optional `gatekeeperFeedback` parameter - see the
// Gatekeeper feedback retry loop in runOrchestrator below. Uses the Gatekeeper's own
// field names verbatim (itemId/ruleViolated/explanation) rather than paraphrasing,
// since Content Writer should be correcting the exact thing that was actually flagged.
function formatGatekeeperFeedback(blockingIssues) {
  return blockingIssues
    .map((issue) => `- Item "${issue.itemId}" violates: ${issue.ruleViolated}\n  Specifically: ${issue.explanation}`)
    .join('\n');
}
...
async function runOrchestrator(db, employeeId, intakeInput = {}, onProgress = null) {
  ...
  const officeTourGuide = resolveOfficeTourGuide(
    db,
    finalMergedContext.employee,
    finalMergedContext.people.humanBuddy,
    finalMergedContext.people.manager ? finalMergedContext.people.manager.email : null
  );
  ...
  // Gatekeeper feedback retry (2026-09-03) - separate from withRetry's per-call
  // JSON-shape retry (which only fires on a genuine thrown exception from either
  // agent, below). A blocking Gatekeeper review is a *successful*, non-throwing
  // response that says "this content has a real problem" - withRetry never sees that
  // as a failure at all, so a blocked plan never used to trigger any retry on its own.
  // Previously: exactly one Content Writer -> Gatekeeper pass, and any blocking issue
  // meant an immediate, final `status: 'blocked'` with no attempt to act on what was
  // actually found. Now: the Gatekeeper's own `issues[]`
  // (itemId/ruleViolated/explanation) are fed back to Content Writer as an explicit
  // correction instruction for a fresh attempt - up to
  // MAX_GATEKEEPER_FEEDBACK_ATTEMPTS additional tries beyond the first plain one. The
  // two retry mechanisms don't share a counter or interact - a JSON failure inside one
  // feedback attempt is handled entirely by withRetry's own budget for that call and
  // never consumes any of this loop's own attempts, and vice versa.
  const MAX_GATEKEEPER_FEEDBACK_ATTEMPTS = 2;
  let content;
  let finalContent;
  let gatekeeperResult;
  let blockingIssues;
  let minorIssues;
  let gatekeeperFeedback = null;

  for (let gkAttempt = 0; gkAttempt <= MAX_GATEKEEPER_FEEDBACK_ATTEMPTS; gkAttempt++) {
    console.log(
      gkAttempt === 0
        ? 'Calling Content Writer agent...'
        : `Calling Content Writer agent again with Gatekeeper feedback (attempt ${gkAttempt}/${MAX_GATEKEEPER_FEEDBACK_ATTEMPTS})...`
    );
    const contentWriterStart = Date.now();
    content = await withRetry('Content Writer', 4, () => runContentWriter(plan, contextForProcessExpert, gatekeeperFeedback), emitRetry);
    ...
    finalContent = withStableItemIds(ensureNoEmptyWeeks(contentWithTracks));
    ...
    gatekeeperResult = await withRetry('Gatekeeper', 4, () => runGatekeeper(finalContent), emitRetry);
    logStageTiming('Gatekeeper', gatekeeperStart);

    blockingIssues = gatekeeperResult.issues.filter((i) => i.severity === 'blocking');
    minorIssues = gatekeeperResult.issues.filter((i) => i.severity !== 'blocking');
    ...
    if (gkAttempt < MAX_GATEKEEPER_FEEDBACK_ATTEMPTS) {
      gatekeeperFeedback = formatGatekeeperFeedback(blockingIssues);
      console.warn(
        `[gatekeeper-feedback] ${blockingIssues.length} blocking issue(s) found (attempt ${gkAttempt + 1}/${MAX_GATEKEEPER_FEEDBACK_ATTEMPTS + 1}) - ` +
          `re-running Content Writer with explicit feedback:\n${gatekeeperFeedback}`
      );
    } else {
      console.warn(
        `Gatekeeper still blocked this plan after ${MAX_GATEKEEPER_FEEDBACK_ATTEMPTS} feedback attempt(s) ` +
          `(${MAX_GATEKEEPER_FEEDBACK_ATTEMPTS + 1} total): ${blockingIssues.length} blocking issue(s) remain.`,
        blockingIssues
      );
    }
  }
  ...
  // A blocking issue that survived every feedback attempt halts the pipeline the same
  // way a direct-report-window violation does above - but this is a content/judgment
  // failure, not a hard scheduling rule, so it doesn't throw. It returns the rejected
  // plan alongside exactly what the Gatekeeper found, so a human can review and decide
  // (fix the prompt, override, or re-run) rather than losing the generated content to
...
module.exports = { runOrchestrator, mergeIntake, attachTracks, ensureNoEmptyWeeks, formatGatekeeperFeedback };
