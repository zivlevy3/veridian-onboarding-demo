...
async function runOrchestrator(db, employeeId, intakeInput = {}) {
  ...
  // Validation is retried together with generation, not as a separate step - a plan
  // that fails the weekly-load/meeting caps isn't a bad generation to patch, it's a
  // candidate to throw away and regenerate (this project's whole real-run history shows
  // a fresh generation usually just fits within the caps on the next attempt).
