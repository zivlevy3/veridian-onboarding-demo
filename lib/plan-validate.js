...
// Support Specialist) on 2026-08-30. Three signals, any one of which is enough:
//   1. Fewer than 8 weeks in `plan.weeks` at all. `PROCESS_EXPERT_TOOL`'s own
//      description requires "exactly 8 entries, weekNumber 1 through 8" but
//      `lib/schemas.js`'s strict-mode JSON schema can't enforce array length
//      (minItems/maxItems aren't supported - documented there as a known limitation),
//      so a 1-week response still passes schema validation. Found for real (Natti
//      Kesem, Product Manager, 2026-08-31): the model returned a single week
//      containing nothing but the "lighter week" placeholder - the original 2 & 3
//      checks below never fire on a 1-element array (there's no way to have "3
//      consecutive empty weeks" to detect when there's only 1 week to look at), so
//      this collapse shape slipped through undetected until this signal was added.
//      This signal alone is enough to flag a collapse, regardless of what's inside
//      whatever weeks did come back - a truncated week count is already a clear
//      structural violation on its own.
//   2. 3+ consecutive fully-empty weeks (each would otherwise render as the "lighter
//      week" placeholder - see orchestrator.js's LIGHTER_WEEK_ITEM/ensureNoEmptyWeeks).
//   3. Zero compliance-track items anywhere in the whole plan. Compliance items
...
// Signals 2 and 3 are combined (both required), same as before - a legitimately light
// back half of a plan should never trip collapse detection on its own. Signal 1 is
// independent and sufficient by itself, since a short weeks array isn't something a
// content-quality judgment call could explain away.
...
function detectPlanCollapse(plan, { minConsecutiveEmptyWeeks = 3, expectedWeekCount = 8 } = {}) {
  const weekCount = plan.weeks.length;
  const tooFewWeeks = weekCount < expectedWeekCount;
  ...
  const emptyRunCollapsed = maxRun >= minConsecutiveEmptyWeeks && !hasComplianceItem;
  const collapsed = tooFewWeeks || emptyRunCollapsed;

  return {
    collapsed,
    maxConsecutiveEmptyWeeks: maxRun,
    hasComplianceItem,
    minConsecutiveEmptyWeeks,
    weekCount,
    expectedWeekCount,
    tooFewWeeks,
  };
